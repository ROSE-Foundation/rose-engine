...
- **Position service (FR-25):** `openPosition`/`closePosition` compose the **existing atomic pair flow** — open drives the real FR-11/FR-18 subscribe+mint path, close drives the real FR-21 redeem/burn path. Every open/close ⇒ a **paired (both-or-neither)** on-chain action; a single-leg path is impossible; capital moves **only** through `postTransfer`.
  ...
  - **Production-faithful mock (Epic 9, §4.9):** `ENGINE_MODE=faithful` may compose a **mock** counterparty/inventory adapter satisfying this guardrail contract so the single-side close completes via re-assignment **in the demo** — the real board-gated §8 Q8 model stays deferred, and the mock never leaks into a real-capital path. [Source: PRD §4.9, FR-31; Sprint Change Proposal 2026-06-20]
