...
> **[NOTE FOR PM — D1 RESOLVED 2026-06-16 (user):** the Rose Note is **separate L/S, zero-sum directional** — L and S are held *separately*, each holder directional (not a single bundled market-neutral package). At a reset the **losing-leg holder bears the locked loss**; the **winning-leg holder is the counterparty**, the gain funded from the shared pool K. **D1a RESOLVED 2026-06-16 (user): crystallised & withdrawable** — at each reset the winner's gain is realized as withdrawable cash, the loser's loss is settled, and both legs re-base to a fresh symmetric split of the residual pool (new neutral cycle, no carried P&L). Each reset is a settlement boundary (cash movement + settlement journal entry); the directional one-leg note shape lands with Epics 4–6 (implementation). The model validation (§4.7) proves the *invariant*, not the *loss-allocation*.]
...
1. **(D1) Rose Note composition & reset loss-allocation** — ~~does one Note bundle both legs (market-neutral holder) or do L/S trade separately (zero-sum, directional)? Who is counterparty to the losing leg's locked loss at a reset?~~ **RESOLVED 2026-06-16 (user): separate L/S, zero-sum directional.** L and S are held separately; each holder is directional. At a reset the **losing-leg holder bears the locked loss** and the **winning-leg holder is the counterparty** (the gain is funded from the shared collateral pool K). **Sub-decision D1a — RESOLVED 2026-06-16 (user): crystallised & withdrawable.** At each reset the winner's gain is realized as **withdrawable cash** and the loser's locked loss is **realized/settled**; both legs then **re-base to a fresh symmetric split** of the residual pool, starting a new neutral cycle (no carried P&L, no compounding). Each reset is thus a **settlement boundary** requiring a cash movement + a settlement journal entry. (The `(K/2)(1±L·r)` formula covers only the issuance/active state; post-reset, the symmetric base is re-established from the residual pool at the new P₀.) The directional one-leg note shape is an implementation item, not an open question — it lands with Epics 4–6.
...
- **Reset loss-allocation (D1 & D1a resolved 2026-06-16).** D1: separate L/S, the losing-leg holder bears the locked loss. D1a: crystallised & withdrawable — each reset realizes the winner's gain (withdrawable) and settles the loser's loss, then both legs re-base to a fresh symmetric split (new neutral cycle). The holder risk is now specified; the residual work is *implementation* of the reset settlement (cash movement + settlement journal entry) in the reset machinery (Epics 5–7). *Mitigation:* SM-2 still scoped to the invariant; reset settlement carries its own balanced-journal + reconciliation tests.
